...
Paper / Folia `26.1.2` 白名单插件, BayMcWhiteList 通过邀请码验证玩家并将白名单状态写入配置的数据库后端, 登录服负责验证, 主服和受保护子服负责拦截未过白玩家

## 🔂 功能特性
- 支持玩家在登录服使用 `/whitelist <邀请码>` 完成白名单验证
- 支持玩家使用 `/whitelist` 自助查询自己的白名单状态
- 支持受保护服务器在预登录阶段查询数据库, 没有白名单记录的玩家会被拒绝加入
- 支持 `login` 登录服模式和 `protected` 受保护服务器模式, 适合多端网络服部署
- 支持 `BAYMC-XXXXXXXX` 格式邀请码, 前缀和后缀长度可配置
- 邀请码后缀使用 `HMAC-SHA256` 按玩家 UUID, 签发日期和前缀计算生成
- 支持七天自然日有效期窗口, 默认检查当天和前 `valid-days - 1` 天
- 统一使用 UUID 作为白名单判断键, 支持 `mojang`, `offline-name`, `server` 三种 UUID 来源
- 支持生成邀请码时按当前 `player.uuid-source` 解析目标身份: Mojang 正版档案, 离线名 UUID 或服务端实际 UUID
- 支持邀请码验证防爆破, 可按玩家和 IP 维度分别开关失败次数限制并临时锁定
- 支持邀请码验证触发限流时通知后台和本服在线管理员
- 支持 MySQL 和 SQLite 作为白名单状态数据源
...
插件依赖 HikariCP, MySQL Connector/J 与 SQLite JDBC, 发布 jar 会通过 Shadow 打包所需运行依赖
...
- 修改 `plugins/BayMcWhiteList/config.yml` 中的存储后端配置; 多服共享推荐 MySQL, 单服或开发环境可使用 SQLite
- 按需修改 `plugins/BayMcWhiteList/lang/zh_CN.yml` 中的 MiniMessage 提示文本
- 将所有服务器的 `code.secret`, `code.prefix`, `code.timezone` 和 `player.uuid-source` 保持一致
...
```yaml
player:
  uuid-source: "mojang"       # 正版服/正确转发正版 UUID 的代理
  # uuid-source: "offline-name" # 纯离线服, 按 OfflinePlayer:<玩家名> 计算 UUID
  # uuid-source: "server"       # 外置登录/代理自定义 UUID, 只信任服务端实际 UUID
```
...
```yaml
storage:
  type: "mysql" # 多服共享白名单状态
```

```yaml
storage:
  type: "sqlite" # 单服, 小服或开发环境
  sqlite:
    file: "whitelist.db"
```
...
- **`/baymcwhitelist info`**<br>
  权限:`baymcwhitelist.info`<br>
  查看插件版本, 服务器模式, 邀请码配置, UUID 来源和数据库状态
- **`/baymcwhitelist help`**<br>
  权限:`baymcwhitelist.help`<br>
  查看详细命令帮助, 包含主命令, 别名, 玩家命令, 管理命令和权限说明

生成邀请码时, 如果目标玩家在线会按当前 UUID 来源解析在线身份;如果不在线, `mojang` 会查询 Mojang 档案, `offline-name` 会按玩家名计算 UUID, `server` 模式必须输入 UUID

## 🧩 邀请码创建提示词

`player.uuid-source: "mojang"` 下, 需要让其他 AI 根据正版玩家名联网查询 UUID 并创建邀请码时, 可以直接使用下面的提示词
...
创建规则
playerUuid = 标准 UUID 字符串
issueDate = timezone 当前日期, 格式 yyyy-MM-dd
payload = playerUuid + ":" + issueDate + ":" + prefix
...
- 白名单状态只写入配置的数据库后端, 不依赖原版白名单文件
- 受保护服务器在数据库不可用时会拒绝玩家加入, 避免误放未知玩家
- 邀请码按玩家 UUID 签名生成, 不能被其他玩家共用
...
- MySQL 表名前缀和 SQLite 文件名都会在加载配置时校验边界
...
- 记录玩家 UUID, 玩家名, 动作类型, 邀请码, 服务器名, IP 和时间
- 覆盖验证成功, 重复验证, 无效邀请码提交, 验证限流, 受保护服务器拒绝加入和管理员移除
- 日志写入当前数据库后端的审计日志表
- 白名单状态写入当前数据库后端的白名单记录表
